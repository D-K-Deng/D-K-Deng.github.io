...
const generateNode = (data: Data, options: Options): Text => {
	...
	if (data.isError) {
		// Failure card behavior
		aProps.onclick = `
      navigator.clipboard.writeText(${JSON.stringify(data.url)})
        .then(() => alert('Copied: ${data.url}'))
        .catch(() => alert('Copy failed, please manually copy'));
    `;
	...
	const children = data.isError
		? [
				...
				h("div", { class: "link-card__info" }, [
					h("div", { class: "link-card__title" }, ["Connection Failed"]),
					h("div", { class: "link-card__description" }, [
						"Click the card to copy the link",
					]),
