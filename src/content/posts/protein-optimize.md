...
The formula for a residue's contribution to the hydrophobic moment vector is:pushpin::
...
The contributions of all residues are summed to yield the 3D hydrophobic moment vector. This resultant vector indicates the overall direction and magnitude of hydrophobicity in the protein structure.

{% katex [displayMode] %}

\text{Hydrophobic Moment Vector} = \sum_{i=1}^{N} (\text{Hydrophobicity}_{i} \times \text{Coordinates}_{i})
